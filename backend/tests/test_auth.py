...
class TestPasswordValidation:
    ...
    def test_password_contains_email(self):
        """測試密碼包含郵箱"""
        result = auth.validate_password_strength_detailed('John123!@#ABC', 'john@example.com', 'John')
        assert not result['valid']
        assert not result['checks']['no_personal_info']

    def test_password_contains_name(self):
        """測試密碼包含姓名"""
        result = auth.validate_password_strength_detailed('Alice123!@#ABC', 'test@example.com', 'Alice')
        assert not result['valid']
        assert not result['checks']['no_personal_info']
    ...
    def test_valid_strong_password(self):
        """測試有效的強密碼"""
        result = auth.validate_password_strength_detailed('MyS3cur3P@ssw0rd!XyZ', '', '')
        assert result['valid']
