...
- **字型系統三軌**：新增 `--font-heading` token（Noto Serif TC），實測後套用範圍縮小到只留側欄品牌名稱；`--mono` 改指向 Source Serif 4，金額/統計數字全面套用 `tabular-nums` 確保直式對齊
- **照片點擊放大檢視**：記一筆待上傳照片、明細編輯記錄、照片瀏覽網格三處縮圖新增點擊放大功能，取代過去縮圖無法放大、只能跳轉編輯視窗的行為

### 🐛 Fixed

- 修改密碼時舊密碼打錯，原本被誤判成「登入已過期」並強制登出，現在正確顯示「舊密碼錯誤」
- 內部轉移原本無餘額檢查、可無限制轉成負數，補上跟支出 `cash_insufficient` 同一套的確認機制；順手修正金額欄位輸入負數的錯誤訊息
- `dashboard.js` 電腦版概覽「分類佔比」卡片引用未定義的 CSS 變數，一直吃 fallback 字面值

### 🔄 Changed

- 清掉 `.app-toast` 死代碼、900px 響應式斷點統一到 `config.js` 單一來源（`DESKTOP_BREAKPOINT`），不再各處各自寫死
