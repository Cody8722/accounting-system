  ...
  wrap.innerHTML = `
    ...
    ${totalBudget === 0
      ...
      : `<div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:7px">
        ...
        <div class="track" style="margin-bottom:var(--space-lg)"><div class="bar" style="width:${Math.min(100, pct)}%;background:${left >= 0 ? 'var(--accent)' : 'var(--expense)'}"></div></div>
...
async function render(container, mode) {
  const title = mode === 'desktop'
    ? '<div class="page-title" style="margin-bottom:var(--space-xl)">預算</div>'
    : `<div style="padding:var(--space-2xs) var(--space-xl) 0;flex-shrink:0">${lockBadgeHtml()}<span style="font-weight:700;font-size:22px;color:var(--text)">預算</span></div>`;
  container.innerHTML = mode === 'desktop' ? `<div class="page">${title}<div class="page-sub">載入中…</div></div>` : `${title}<div style="padding:var(--space-empty-state);text-align:center;color:var(--muted2)">載入中…</div>`;
  let data;
  try { data = await fetchData(); } catch (e) { container.innerHTML = `<div style="padding:var(--space-empty-state);text-align:center;color:var(--expense)">${escapeHtml(e.message)}</div>`; return; }
  ...
  inner.innerHTML = `
    <div style="background:var(--accent);border-radius:22px;padding:var(--space-xl) 22px;color:#fff;margin-bottom:var(--space-xl)">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-xs)">
        ...
        <button data-el="edit" style="border:none;background:rgba(255,255,255,.18);color:#fff;border-radius:9px;padding:5px var(--space-base);font-size:13px;cursor:pointer"><i class="ti ti-adjustments"></i> 設定</button>
      </div>
      <div style="display:flex;align-items:flex-end;gap:var(--space-2xs);margin-bottom:var(--space-emphasis)">
    ...
    <div data-el="rows">${cats.map((c) => rowHtml(c, data.budget[c], data.spentMap[c] || 0)).join('') || '<div style="text-align:center;color:var(--muted2);padding:var(--space-xl)">尚未設定任何分類預算</div>'}</div>`;
